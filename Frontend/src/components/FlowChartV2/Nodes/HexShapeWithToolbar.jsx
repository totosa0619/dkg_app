...
function HexShapeWithToolbar({ id, data, selected, isEditMode }) {
  const onChange = (_evt) => {
    data.handleEdit(id);
  };
  ...
  const effectiveWidth = width - 4; // Subtracting for stroke and translation margin
      ...
      >
        <svg width={width} height={height} className="shape-svg">
          <g transform={`translate(2, 2)`}>
            <path
              d={`M0,${effectiveHeight * 0.467} L${
                effectiveWidth * 0.09667
              },0 L${effectiveWidth * 0.90333},0 L${effectiveWidth},${
                effectiveHeight * 0.467
              } L${effectiveWidth * 0.90333},${effectiveHeight} L${
                effectiveWidth * 0.09667
              },${effectiveHeight} Z`}
...
export default HexShapeWithToolbar;
