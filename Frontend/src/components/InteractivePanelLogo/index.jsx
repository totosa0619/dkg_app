...
const Arc = ({ data, index, createArc, setTextData, setTitle }) => {
  ...
  var infoX = Math.sin(data.midAngle) * infoDist;
  var infoY = -Math.cos(data.midAngle) * infoDist;
...
function InteractivePanelLogo(props) {
  ...
  const mainLogoUrlProps = props?.data.originalData.mainLogoUrl;
  ...
  const getUniqueItems = (data) => {
    const uniqueLabelsSet = new Set(data.flatMap(col => col.map(item => item?.label).filter(Boolean)));
    const uniqueLabelsArray = Array.from(uniqueLabelsSet);
    const uniqueItems = uniqueLabelsArray.map(label => {
      const itemsWithLabel = data.flatMap(col => col.filter(item => item?.label === label));
      return itemsWithLabel[0];
    });
    return uniqueItems;
  };
          ...
          <div className={classes.interactivePanelGridBody}>
            {rightPanelData
              ? rightPanelData.map((col, i) => {
                const uniqueCol = getUniqueItems([col]);
                  return (
                    <div
                      key={"col" + i}
...
export default InteractivePanelLogo;
