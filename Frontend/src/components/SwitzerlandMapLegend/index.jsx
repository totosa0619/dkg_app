...
const CustomPoint = ({
  ...
  return geoData?.logos?.length || geoData?.description ? (
    ...
    >
      ...
      <foreignObject width={100} height={descriptionHeight} y={250} x={165}>
        <div
          style={{
            width: "100%",
            height: "100%",
...
const SwitzerlandMapLegend = (props) => {
  ...
  const [hoverArea, setHoverArea] = useState("Zürich");
  ...
  const areaWithCoordinates = useMemo(() => {
    ...
    areasList.forEach((el) => {
      const area = TOPO_JSON.objects.switzerland.geometries.find((element) => {
...
export default SwitzerlandMapLegend;
