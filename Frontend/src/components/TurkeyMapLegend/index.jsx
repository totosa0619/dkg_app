...
const CustomPoint = ({
  ...
  return geoData?.logos?.length || geoData?.description ? (
    ...
    >
      ...
      <foreignObject width={250} height={descriptionHeight} y={-20} x={165}>
        <div
          style={{
            width: "100%",
            height: "100%",
        ...
        >
          <span className={classes.contentBox}>
            <span style={{ marginBottom: "3px", fontSize: "45px" }}>
              {name}
            </span>
...
const TurkeyMapLegend = (props) => {
  ...
  const [hoverArea, setHoverArea] = useState("Istanbul");
  ...
  const areaWithCoordinates = useMemo(() => {
    ...
    areasList.forEach((el) => {
      const area = TOPO_JSON.objects.turkey.geometries.find((element) => {
        return element?.properties?.woe_name === el;
      });
      if (area?.properties?.longitude && area?.properties?.latitude) {
...
export default TurkeyMapLegend;
